'use strict';
app.service('boxService', [function() {
	
	function ajax( options ){
		$.ajax({
			url:options.url,
			data:options.data,
			dataType: options.dataType ? options.dataType : "json",
			type: options.type ? options.type : "post",
			success: options.success,
	    	contentType : options.contentType ? options.contentType : "application/x-www-form-urlencoded",
			error: options.error ? options.error : function(){}
		}).then(function( data ){
			if( options.then ){
				options.then.call( this,data );
			}
		});
	}
	
	function save( options ){
		var data = options.data;
		
	    if( data.boxNo == ""){
			throw new BusinessException("boxNo不能为空");
		}
	    if( data.weight == ""){
			throw new BusinessException("weight不能为空");
		}
	    if( data.openBox == ""){
			throw new BusinessException("openBox不能为空");
		}
	    if( data.type == ""){
			throw new BusinessException("type不能为空");
		}
	    if( data.imagePath == ""){
			throw new BusinessException("imagePath不能为空");
		}
	    if( data.objectId == ""){
			throw new BusinessException("objectId不能为空");
		}
	    if( data.createTime == ""){
			throw new BusinessException("createTime不能为空");
		}
	    if( data.sortingCategoryId == ""){
			throw new BusinessException("sortingCategoryId不能为空");
		}
		
		options.data = {
			boxId:data.boxId,
			boxNo:data.boxNo,
			weight:data.weight,
			openBox:data.openBox,
			type:data.type,
			imagePath:data.imagePath,
			objectId:data.objectId,
			createTime:new Date( data.createTime ).format('yyyy-MM-dd HH:mm:ss'),
			sortingCategoryId:data.sortingCategoryId,
		}
		options.url = urls.ms+"/flower/box/save.do";
		ajax( options );
	}
	
	function getById( options ){
		options.url = urls.ms+"/flower/box/get.do";
		ajax( options );
	}
	
	function remove( options ){
		options.url = urls.ms+"/flower/box/delete.do";
		ajax( options );
	}
	
	return {
		remove:remove,
		getById:getById,
		save:save
	};
}]);