...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		updatepager = function(rn, dnd) {
			...
			var options = $( ts ).jqGrid("getGridParam");
			if( $( ts ).jqGrid("getGridParam").autoCompile ){
				try{
					angular.element( ts ).injector().invoke(function( $compile ) {
						$compile( $( ts ).find( ".td-compile" ) )( angular.element( ts ).scope() );
					});
				}catch (e) {
					console.error( e );
				}
			...
			window.onresize=function(){
				$( ts ).jqGrid('setGridWidth', $( ts ).parents( ".ui-jqgrid:eq(0)" ).width() );
			}
				...
				case "script":
					$.ajax($.extend({
						...
						success:function(data,st, xhr) {
							if( st != "success" || data.success == false || (
							...
							var rowNum = $( ts ).jqGrid("getGridParam").rowNum;
							var page = $( ts ).jqGrid("getGridParam").page;
