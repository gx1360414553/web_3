...
;(function(window, document, angular, undefined){
	angular.module('ng-nestable', [])
		...
		.directive('ngNestable', ['$compile', '$nestable', function($compile, $nestable){
				...
				compile: function(element){
					...
					return function($scope, $element, $attrs, $ngModel){
						...
						var options = $.extend(
							...
							(new Function('return ' + "{"+$attrs.ngNestable +"}" ))()
						);
						$scope.$watchCollection(function(){
							return $ngModel.$modelValue;
						}, function(model,oldModel,$scope){
							if(model && element.is(':empty')){
								...
								var root = buildNestableHtml(model, itemTemplate,$scope);
			...
			function buildNestableHtml(model, tpl,$scope){
				...
				$(root).find("label").on("click",function( event ){
					var $this = $(this);
					if( event.target.type == "checkbox" ){
						...
						if( $item.checked ){
							...
							$parents.each(function(i){
								angular.element( this ).scope().$item.checked=true;
							});
						}else{
							var $parents = $this.parents(".dd-list").prev("div");
							$parents.each(function(i){
								var parent = angular.element( this ).scope().$item;
						...
						if( $item.children ){
							$item.children.forEach(function f( child ){
